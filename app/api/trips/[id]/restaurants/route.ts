...
export async function GET(
  ...
  try {
    ...
    if (error) {
      logger.error('api/restaurants', 'Supabase error on GET', { error: error.message, recordId: tripId })
      return NextResponse.json({ error: error.message }, { status: 500 })
    }
    return NextResponse.json(data)
  } catch (err) {
    logger.critical('api/restaurants', 'Unhandled exception in GET handler', { error: err instanceof Error ? err.message : String(err) })
...
export async function POST(
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .single()
    if (error) {
      logger.error('api/restaurants', 'Supabase error on POST', { error: error.message, recordId: tripId })
      return NextResponse.json({ error: error.message }, { status: 500 })
    }
    return NextResponse.json(data, { status: 201 })
  } catch (err) {
    logger.critical('api/restaurants', 'Unhandled exception in POST handler', { error: err instanceof Error ? err.message : String(err) })
    return NextResponse.json({ error: 'Unexpected server error' }, { status: 500 })
  }
}
