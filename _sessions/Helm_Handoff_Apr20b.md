...
## Current Version
`00.01.0121` — local only (not yet pushed to production)
...
| Calendar | 🔧 In progress — Block 2 complete; gcal_include architecture approved, not yet built | 00.01.0121 |
...
## Next Session
**START HERE: Build the gcal_include opt-in architecture (full plan below)**

### Calendar Test Plan — Status

Block 1 — Connection & Disconnection: ✅ Complete (5/5 passed; 1.6–1.7 deferred to Block 7)
Block 2 — Dirty Flag Behavior: ✅ Complete (all steps passed; bugs F7 found and resolved)
Blocks 3–8: **On hold** pending gcal_include architecture build.

### Bugs resolved this session
| Bug | Description | Resolution |
|---|---|---|
| F2 | `getAuthUserId()` broken in flights and hotels PATCH routes | Fixed |
| F3 | CalendarButton not re-fetching status after save | Fixed via `gcal:dirty` CustomEvent + `useEffect` listener |
| F4 | Import Document button visually dominant | Changed to outline/secondary style |
| F5 | CalendarButton State 3 styling | `action` variant in Button.tsx, 4px amber border, fills on hover, bold "Update!" sub-label |
| F6 | `--action` and `--action-text` tokens missing | Added to globals.css |
| F1 | Status fetch sequential | Parallelized via `Promise.all` |
| F7 | Checklist push not clearing dirty flag on null-due-date records | pushSection.ts Pass 2 added; query fixed to two clean passes |

### Architecture decision: gcal_include opt-in model
The implicit dirty-flag model (app decides what goes to calendar) is replaced with an explicit per-record opt-in. Users decide per record what goes on their calendar via a checkbox in each section's BottomSheet edit form. The app disables the checkbox when the record lacks minimum data for a valid calendar event.

---

## gcal_include Architecture Plan

### 1. Schema Changes

**New column on all 6 tables:**
```sql
...
**Migration — reset all existing gcal state:**
```sql
...
### 2. Per-Section Validity Rules
Checkbox rendered in all section BottomSheets. Disabled until minimum data for a valid calendar event is present.

| Section | Checkbox enabled when… | Reason |
|---|---|---|
| Flights | Always enabled | Dates are effectively required at creation |
| Hotels | Always enabled | Check-in/out dates are effectively required at creation |
| Transportation | `departure_date` is set | No date = no event |
| Restaurants | `reservation_date` is set | No date = no event |
| Itinerary | `activity_date` is set | All-day valid; no time required |
| Checklist | `due_date` is set | No date = no event |

Disabled checkbox renders with helper text explaining why (e.g. "Set a due date to enable calendar sync").

### 3. API Changes

**All 6 PATCH routes:**
1. Accept `gcal_include` in the payload
2. Set `gcal_dirty = true` only when `gcal_include = true`; set `gcal_dirty = false` when `gcal_include = false`

**All 6 POST routes:**
- New records always start with `gcal_include = false` and `gcal_dirty = false`

### 4. Push Handler Changes (`pushSection.ts`)

Pass 2 (skip-and-clear path) eliminated entirely. Push query simplifies to:
```
gcal_include = true AND gcal_dirty = true
```

### 5. UI Changes

Each section BottomSheet — add at the bottom of the form, above Save:
```
[ ] Add to Google Calendar
```
- Checked = `gcal_include: true`
- Unchecked = `gcal_include: false`
- Disabled (greyed) when validity rule not met, with helper text
- Initialised from the record's current `gcal_include` value on edit
- New records always initialise unchecked

### 6. Status Check (`/api/gcal/status`)
No change needed. Dirty flag is now only ever set when `gcal_include = true`, so the count is inherently correct.

### 7. Files Affected

| File | Change |
|---|---|
| **Schema** | 6 `ALTER TABLE` + 6 `UPDATE` migration statements |
| `app/api/flights/[id]/route.ts` | PATCH: accept `gcal_include`; conditional dirty flag |
| `app/api/hotels/[id]/route.ts` | PATCH: accept `gcal_include`; conditional dirty flag |
| `app/api/transportation/[id]/route.ts` | PATCH: accept `gcal_include`; conditional dirty flag |
| `app/api/restaurants/[id]/route.ts` | PATCH: accept `gcal_include`; conditional dirty flag |
| `app/api/itinerary/rows/[rowId]/route.ts` | PATCH: accept `gcal_include`; conditional dirty flag |
| `app/api/checklist/[id]/route.ts` | PATCH: accept `gcal_include`; conditional dirty flag |
| `components/sections/FlightsClient.tsx` | BottomSheet: add checkbox; include in payload |
| `components/sections/HotelsClient.tsx` | BottomSheet: add checkbox; include in payload |
| `components/sections/TransportationClient.tsx` | BottomSheet: add checkbox; include in payload |
| `components/sections/RestaurantsClient.tsx` | BottomSheet: add checkbox; include in payload |
| `components/advisor/ItineraryClient.tsx` | BottomSheet: add checkbox; include in payload |
| `components/sections/ChecklistClient.tsx` | BottomSheet: add checkbox; include in payload |
| `app/api/gcal/push/_shared/pushSection.ts` | Simplify query; remove Pass 2 |

**15 files total** — 6 API routes, 6 client components, 1 push handler, schema migration, version bump.

### 8. Build Order
1. Schema migration (Stan runs in Supabase)
2. Push handler simplification
3. API routes (6)
4. Client components (6)
5. Version bump
6. Localhost testing — verify checkbox saves, dirty flag triggers correctly, push processes only opted-in records
...
- **gcal_include = true** required for a record to be pushed (new model)
- Dirty flag (`gcal_dirty = true`) set on save only when `gcal_include = true`
...
|---|---|---|---|
| 1 — Unconnected | No token or no gcal_calendar_id | "Unconnected" | Outline (secondary variant) |
| 2 — Connected | Token + calendar, no dirty records | "✅ Connected" | Outline (secondary variant) |
| 3 — Update Required | Token + calendar + dirty records > 0 | "Update!" (bold) | 4px amber border (`#E07B00`), fills on hover; `action` variant |

### Action token
- `--action: #E07B00` — system-wide "needs attention" color
- `--action-text: #ffffff`
- Used in `Button.tsx` `action` variant: transparent bg + 4px amber border unhovered; amber fill on hover
- Sub-label "Update!" uses `fontWeight: 700`; button shell uses `fontWeight: 400`
- On iOS/touch: hover fill never triggers — amber border is the sole attention signal
...
### Dirty flag logic (updated)
- `gcal_include = false` → never dirty, never pushed
- `gcal_include = true` + `gcal_event_id = null` → create on next push
- `gcal_include = true` + `gcal_dirty = true` + existing id → update on next push
- `gcal_include = true` + `gcal_dirty = false` + existing id → skip
...
| `app/api/gcal/status/[tripId]/route.ts` | Returns unconnected / connected / update_required + dirty count; queries parallelized via `Promise.all` |
...
| File | Change |
|---|---|
| `components/advisor/TripDetailView.tsx` | CalendarButton rendered after Print Trip; Import Document changed to `variant="secondary"` (outline) |
| `app/api/flights/[id]/route.ts` | PATCH fixed: `getAuthUserId()` restored; adds `gcal_dirty: true` |
| `app/api/hotels/[id]/route.ts` | PATCH fixed: `getAuthUserId()` added (had none); adds `gcal_dirty: true` |
...
| `components/sections/FlightsClient.tsx` | Dispatches `gcal:dirty` CustomEvent on save |
| `components/sections/HotelsClient.tsx` | Dispatches `gcal:dirty` CustomEvent on save |
| `components/sections/TransportationClient.tsx` | Dispatches `gcal:dirty` CustomEvent on save |
| `components/sections/RestaurantsClient.tsx` | Dispatches `gcal:dirty` CustomEvent on save |
| `components/advisor/ItineraryClient.tsx` | Dispatches `gcal:dirty` CustomEvent in `handleSaveRow` only |
| `components/sections/ChecklistClient.tsx` | Dispatches `gcal:dirty` CustomEvent on save |
| `components/ui/Button.tsx` | Added `action` variant; added `borderWidth` and `fontWeight` fields to `VariantTokens` |
| `app/globals.css` | Added `--action: #E07B00` and `--action-text: #ffffff` to Semantic block |
...
### Apr 20 (morning)
- Calendar OAuth `redirect_uri_mismatch` confirmed resolved — full OAuth flow working
- Calendar pushed to production: 00.01.0111
- Calendar test plan defined (8 blocks)
- Block 1 (Connection & Disconnection): ✅ 5/5 passed
- Block 2 (Dirty Flag): partially tested — dirty flag writing confirmed via SQL; CalendarButton state transition broken, fixed below
- Bugs found and fixed:
  - F2: `getAuthUserId()` broken in flights and hotels PATCH routes — fixed
  - F3: CalendarButton not re-fetching status after save — fixed via `gcal:dirty` CustomEvent + `useEffect` listener
  - F4: Import Document button visually dominant — changed to outline/secondary style
  - F5: CalendarButton State 3 styling — resolved through several iterations; final: `action` variant in Button.tsx, 4px amber border (`#E07B00`) unhovered, fills on hover, bold "Update!" sub-label
  - F6: `--action` and `--action-text` tokens added to globals.css
  - F1: Status fetch parallelized via `Promise.all` — confirmed faster
- Versions 00.01.0112–00.01.0119 pushed to production

### Apr 20 (afternoon)
- Block 2 resumed and completed ✅
- Bug F7: Checklist push not clearing dirty flag on null-due-date records — found and fixed
  - pushSection.ts rewritten to use two clean queries (Pass 1: records with due_date; Pass 2: dirty records without due_date)
  - Version 00.01.0120 → 00.01.0121
- Architecture decision: implicit dirty-flag model replaced with explicit gcal_include opt-in
  - Rationale: user should decide what goes on calendar; implicit model creates confusion when dateless records don't produce events
  - Full architecture plan documented above
  - Blocks 3–8 on hold pending gcal_include build
