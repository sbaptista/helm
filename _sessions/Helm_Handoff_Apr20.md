...
## Current Version
`00.01.0119` — pushed to production
...
| Calendar | 🔧 In progress — OAuth working, Blocks 1–2 partial testing done | 00.01.0119 |
...
## Next Session
**START HERE: Resume Calendar testing at Block 2**

### Calendar Test Plan — Remaining Blocks

Block 1 — Connection & Disconnection: ✅ Complete (5/5 passed; 1.6–1.7 deferred to Block 7)

Block 2 — Dirty Flag Behavior: In progress. Dirty flag confirmed writing correctly (AS 900 showed `gcal_dirty = true`). CalendarButton now transitions to State 3 on save without page reload (F3 fix). Resume Block 2 testing on localhost with all fixes applied.

**Blocks 3–8 not yet started:**
- Block 3 — Push / Upsert
...
### SQL for dirty flag inspection (replace `'your-trip-id'` with actual UUID)
```sql
SELECT flight_number, gcal_dirty FROM flights WHERE trip_id = 'your-trip-id';
SELECT id, gcal_checkin_event_id, gcal_checkout_event_id, gcal_dirty FROM hotels WHERE trip_id = 'your-trip-id';
SELECT id, gcal_event_id, gcal_dirty FROM transportation WHERE trip_id = 'your-trip-id';
SELECT id, gcal_event_id, gcal_dirty FROM restaurants WHERE trip_id = 'your-trip-id';
SELECT id, gcal_event_id, gcal_dirty FROM itinerary_rows WHERE itinerary_id IN (SELECT id FROM itineraries WHERE trip_id = 'your-trip-id');
SELECT id, gcal_due_event_id, gcal_warning_event_id, gcal_dirty FROM checklist WHERE trip_id = 'your-trip-id';
```
