...
## Current Version
`00.02.0004` — pushed to production
...
| 15 | All sections | Required field validation — holistic red-border system designed. FormField component + Flights reference implementation building now. Remaining: Hotels, Itinerary, Checklist, Restaurants, Transportation, Key Info. | In progress |
...
## Next Session
1. **Issue #15 continued** — Required field validation rollout: Hotels, Itinerary, Checklist, Restaurants, Transportation, Key Info (Flights + FormField being built this session).

---

## Required Field Validation System (in progress — 00.02.0005+)

### Design (approved)
- **Trigger:** First save attempt only — dormant until user tries to submit. Less jarring on form open.
- **Visual:** Red border around the required input + small "Required" text below the field
- **Persistence:** Red clears as soon as the field is filled (reactive, not waiting for another save attempt)
- **Save block:** Save is blocked when any required field is empty after first attempt
- **Asterisk:** `*` label indicator stays as a pre-attempt hint that the field is required

### What it replaces vs. keeps
- **Replaces:** Simple required-field toasts ("Flight number is required", "Airline name is required", "Title is required", etc.)
- **Keeps:** Conditional/cross-field validation toasts ("Arrival must be after departure", "Date required when time is set") — relational rules that cannot be represented as a simple empty-field indicator
- **Keeps entirely:** WARN system (WarnBadge/WarnBanner on cards, `action_required`) — record-level concern, not form validation

### Technical approach
- `FormField` component (`components/ui/FormField.tsx`) extended with `attempted` boolean prop
- When `attempted && required && field is empty`: red border on input + "Required" text below
- Each form gets a single `attempted` state, initialized `false`, set to `true` on first save attempt
- `--field-error` CSS custom property already present on FormField wrapper div — consumers use it for red border

### Implementation scope
| Section | Status |
|---|---|
| FormField component | Building this session |
| Flights | Building this session (reference implementation) |
| Hotels | Pending |
| Itinerary | Pending |
| Checklist | Pending |
| Restaurants | Pending |
...
- **Three-layer architecture:**
  - Layer 1: 3 shooting stars (`position: fixed`) — top-right → bottom-left, JS-controlled random dwell, varying angles (-36° / -38° / -43°), varying tail lengths
...
- **Version string:** bottom-left, `left: 16px`

### Shooting star timing (JS-controlled)
- CSS `infinite` replaced with JS `animationend` + random dwell loop
...
- `TripDetailView` split into outer wrapper + `TripDetailViewInner` to satisfy rules of hooks. Outer wrapper owns `useOnlineStatus` + early return to `<OfflinePage />`. `ToastProvider` wrap preserved.
- `ErrorBoundary` (class component) wraps `{children}` in `app/layout.tsx` — catches fatal render errors app-wide, renders `<FatalErrorPage />`
- `ServiceWorkerKiller` — unregisters any stale service workers on mount, also in `app/layout.tsx`
- `transform-box: fill-box; transform-origin: center center` on wheel `.W` group (both components)
- CSS namespaced: `hop-` prefix (offline), `hep-` prefix (error) — no globals impact
...
- **Required field validation**: `attempted` state pattern — set to `true` on first save attempt only. Never show red borders on untouched forms. Conditional/cross-field toasts (arrival after departure, etc.) are separate and stay as toasts.
...
- **Required field validation** — red border + "Required" text below field, triggered on first save attempt only. `FormField` `attempted` prop is the mechanism. Replaces simple required-field toasts; keeps conditional/cross-field toasts and WARN system.
...
- **Issue #15 redesigned:** Required field validation overhauled from serial toasts to holistic red-border system. Design approved. `FormField` + Flights reference implementation building this session. Version 00.02.0005.
