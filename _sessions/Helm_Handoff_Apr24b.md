...
## Current Version
`00.01.0153` — pushed to production
...
## Next Session
**Search** — full-site search across all trips and sections. Architecture planned, not yet built. See Search Architecture section below.

Other candidate items (lower priority):
...
## Search Architecture (planned, not built)

### Decisions made
- **Scope:** Entire app — all trips, all sections
- **Entry point:** Search bar in top-level nav, visible on all pages including Dashboard
- **Results display:** Full results page/view (not inline dropdown) — may revisit
- **Searchable content:** Text fields AND structured data (dates, statuses, `action_required` flags etc.)
- **Result click behavior:** Navigate directly to the record — correct trip, correct section, record BottomSheet opened
- **Logs in search:** Included as a separate filter, not mixed with trip content results

### The hard problem: deep linking to a record
Clicking a search result must open the correct trip, navigate to the correct section, and open that record's BottomSheet. Currently the app has no mechanism for this — tabs are local state, BottomSheets open on user interaction only.

**Agreed approach:** URL-driven navigation. Search results link to `/trips/[id]?section=flights&record=abc123`. The trip page reads those params on load and opens the correct section and record automatically.

### Search execution: server-side
Client-side filtering across multiple trips and sections would be unwieldy. A dedicated server-side API route queries Supabase across all section tables, returning ranked results with trip + section + record metadata.

### Rough component architecture
1. `SearchBar` component in top nav — input, triggers navigation to `/search?q=...&trip=...&section=...`
2. `/search` page — sends query to API, displays full results view with filters
3. `app/api/search/route.ts` — queries all section tables, returns results with trip + section + record metadata
4. Result click → `/trips/[id]?section=flights&record=abc123` → trip page reads params, opens section and record

### Not yet decided
- Result ranking/scoring
- Exact filter UI (All / By trip / By section / Logs)
- How BottomSheet deep-link open is implemented (URL param read on mount)
- Whether to search Logs table or treat it separately
...
### Fatal logging note
Fatal means Supabase failed to initialize — `logger.ts` depends on Supabase and likely can't write either. The backstop is `console.error` in `logger.ts`, visible in Vercel function logs. A separate logging service would be required for guaranteed Fatal logging — overkill for Helm.
...
### Future refinement
`PersistentMessage` currently has hardcoded copy. Should eventually accept optional `title` and `body` props for context-specific messaging, with hardcoded strings as fallback defaults.
...
### New files
- `lib/logger.ts` — server-side only, fire-and-forget, 4 levels (warn/error/critical/fatal), logs to `helm_logs` via service role client, `console.error` backstop on logging failure. Uses `SUPABASE_SECRET_KEY` env var (not `SUPABASE_SERVICE_ROLE_KEY`).
...
- `components/ui/DevDebugPanel.tsx` — dev-only floating panel (bottom-right, `NODE_ENV === 'development'` guard). DEV toggle button, three buttons: Trigger Error (real fetch to non-existent endpoint → toast), Trigger Critical (portal to `document.body`, fixed top of viewport), Trigger Fatal (portal to `document.body`, full-screen overlay). Wrapped in its own `ToastProvider`.
- `components/sections/LogsClient.tsx` — full logs viewer. Level filter, source filter (dynamic from data), expand/collapse rows, clear by threshold with confirmation.
...
### Logs section
- Accessed via "Logs" link in trip header — renders as standalone view replacing tab bar and section content
- Reverse chronological list
- Filter by level (ALL / WARN / ERROR / CRITICAL / FATAL)
- Filter by source — dynamic, derived from fetched data
- Trip-scoped: current trip logs + null `trip_id` entries
- Each row: timestamp, LevelBadge, source, message — expandable to show payload
- Clear logs by threshold: 7 / 30 / 90 days with inline confirmation
...
- Logs moved out of section nav. Now lives as "Logs" link in trip header actions area (alongside Edit trip, Clear trip data, Delete trip). Opens as standalone view — tab bar and section content replaced entirely. Back button returns to previous tab. Tab system has zero awareness of Logs. Version 00.01.0149–00.01.0153.
- Dev Debug Panel built: `components/ui/DevDebugPanel.tsx`, dev-only, floating bottom-right, three buttons (Trigger Error / Critical / Fatal). Critical and Fatal use `createPortal` to render outside panel DOM. Version 00.01.0150–00.01.0151.
- Fatal error page redesigned: dark `#111111` full-viewport background, centered card with ShieldX icon, "Fatal error" eyebrow, "App error" headline, body text, white Reload button. Version 00.01.0152.
- Open Issue #16 added: Fatal error page dark theme feels foreboding — design revisit deferred.
- Search architecture planned (not built). See Search Architecture section above.
