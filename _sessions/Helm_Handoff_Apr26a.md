...
## Current Version
`00.02.0002` — pushed to production
...
| Logs | ✅ Complete — Phase 1–4 done. Triggered from sidebar APP group. | 00.01.0156 |
...
| 15 | Required field indicators | Visual indication of required fields across all section forms. Design deferred. | Next session |
| 16 | Fatal error page | Designed — HTML mockup complete. Wiring into Helm pending. | Wire next session |
...
## Next Session
1. **Wire offline page** into Helm:
   - Disable unintentional service worker
   - Build `OfflinePage.tsx` component from approved HTML mockup
   - Replace `navigator.onLine` / event listener approach with active polling against `/api/health` (every 10s)
   - Gate entire app — when offline, render `OfflinePage` instead of normal app
   - Auto-resume when connection returns (no manual retry button)
   - Testing: cut network via Mac/iPhone Wi-Fi settings OR Chrome DevTools → Network → Offline. Dev Debug Panel (DEV button) for general error confirmation.

2. **Wire fatal error page** into Helm:
   - Build `FatalErrorPage.tsx` from approved HTML mockup
   - Render when a fatal/unrecoverable error is caught
   - Includes: Reload button, JS timestamp, status pill "Error"
   - Technical details surface in Logs section, not on the error page

3. **Issue #15** — Required field indicators: design conversation first, then build.

---

## Offline / Error Page System (designed this session)

### Visual design (approved)
- **Background:** Dark navy `#0A1B2E`
- **Three-layer architecture:**
  - Layer 1: 3 shooting stars (`position: fixed`) — top-right → bottom-left, fire at independent intervals (1s / 7.5s / 14s delays), varying angles (-36° / -38° / -43°), varying tail lengths
  - Layer 2: ~26 twinkling gold stars (`position: fixed`, vw/vh positioned) — spread across full viewport including mid-screen fill
  - Layer 3: Content unit — cream wheel (opacity 0.20, `transform-box: fill-box` rotation), north star, HELM wordmark, text, divider, status pill
- **Fonts:** Cormorant Garamond (headlines) + Lato (body) — same as app
- **Wheel:** Cream `#E8E2D8`, cropped viewBox `190 110 300 300`, displayed at `min(440px, 88vw)`

### Two scenarios

**Offline (`OfflinePage.tsx`):**
- Title: "Taking a breather"
- Details: "Helm needs an internet connection to keep your trip data safe and in sync. We'll be right back as soon as you're connected again."
- Caveat: "If you were mid-edit when this happened, that change may not have been saved. We're sorry — offline editing is on the roadmap."
- No retry button — auto-resumes when connection returns
- Status pill: "● Offline"

**Fatal Error (`FatalErrorPage.tsx`):**
- Title: "Something went wrong"
- Details: "Helm ran into an unexpected problem. Your trip data is safe — nothing that was already saved has been affected."
- Caveat: "If you were editing something when this happened, that change may not have been saved. We're sorry — that one's on us."
- **Reload button:** `window.location.reload()`, subtle cream-bordered styling
- **Timestamp:** JS-generated at render time ("Occurred at 2:14 PM")
- Status pill: "● Error"
- Technical details surface in Logs section — error page stays clean

### HTML mockup files
- `helm-offline.html` — approved offline page
- `helm-error.html` — approved fatal error page

### Detection approach (to be implemented)
- Replace `navigator.onLine` + event listeners (unreliable on iOS) with active polling
- Poll `/api/health` every 10 seconds
- On failure: render `OfflinePage`
- On recovery: auto-resume, no user action required
- Fatal error: caught by React error boundary or top-level try/catch
...
- `checklist_items` → `checklist` (wrong table name)
- Added `.is('deleted_at', null)` to checklist, key_info, and packing queries
...
### Performance (fixed 00.02.0001)
- `page.tsx`: trimmed from 14 to 9 Supabase queries — 5 full data fetches removed
- Print data now fetched lazily inside `PrintExportModal` on open
- `router.prefetch('/advisor/dashboard')` added on mount in `TripDetailView`
...
- **Version string** — `rgba(242,237,228,0.45)` on dark backgrounds for visibility
- **Offline/Error pages** — night scene design system (dark navy, cream wheel, gold stars, shooting stars). Clean human copy, no technical details. Logs section handles technical detail.
...
- **Issues #7, #8, #9, #12, #13** batch fix:
  - #7: Sticky offline banner added to `TripDetailView.tsx`.
  - #8: Hotels "Hotel Name *" → "Hotel Name"; "Province" → "State / Province" with correct placeholder.
  - #9: Native `title` tooltips added to all 7 section form files.
  - #12: gcal_include checkbox standardised to 20×20px + gold accent across all 6 sections.
  - #13: Hotels gcal_include disabled state added (`disabled={!form.check_in_date}`, opacity fade, helper text). Itinerary confirmed already wired.
  - Version 00.02.0002. Pushed to production.
- **Issues #4, #5, #14** closed (no code change — design decisions).
- **Issue #22** added: iOS touch freeze, cannot reproduce on demand, monitor.
- **Offline/Error page system designed:**
  - Art direction brief written.
  - Night scene design with three-layer architecture approved (shooting stars, twinkling stars, content unit).
  - Helm wheel chosen over compass as the background element.
  - `helm-offline.html` and `helm-error.html` mockups approved.
  - Wiring into Helm is next session's first task.
