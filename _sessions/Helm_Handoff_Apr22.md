...
## Current Version
`00.01.0131` — local only, not yet pushed to production
...
| Restaurants | ✅ Full redesign complete — pending final card test | 00.01.0131 |
...
| 11 | Button.tsx | ✅ RESOLVED this session — `borderColor` shorthand replaced with four `borderXxxColor` longhands across Button.tsx + 7 section client files | Closed |
...
## Next Session
**START HERE: Complete Restaurants card test (Test C), then push**

### Restaurants Card Test — C (remaining)
- Confirm status badge shows "✅ CONFIRMED" when confirmed, "Reservation: [status]" otherwise
- Confirm display label appears as subtitle below name
- Confirm Style shows with 2-line clamp
- Confirm Maps ↗ and View Booking ↗ render as links, not raw URLs
- Confirm Notes shows with 3-line clamp
- Confirm Action Required banner shows when flagged

### After Restaurants is confirmed — push `00.01.0131` or `00.01.0132`

### Then — continue CAN26 → Helm parity audit
Parity work completed this session:
- ✅ Flights — full parity build
- ✅ Transportation — timezone fields added
- ✅ Hotels — province, postal code, maps_url added
- ✅ Restaurants — full redesign

Remaining sections to audit:
- Itinerary — compare against CAN26 itinerary rows
- Checklist — compare against CAN26 todos
- Packing — already at parity (confirmed this session)
- Key Info — Helm addition, no CAN26 equivalent

### Design decisions established this session (apply going forward)
...
## Parity Audit — Schema Changes Applied This Session
...
|---|---|---|
| F8 | Flights PATCH 401 | Fixed in `app/api/flights/[id]/route.ts` |
| F8b | Flights POST 401 | Fixed in `app/api/trips/[id]/flights/route.ts` |
| F8c | Flights DELETE 401 | Fixed in `app/api/flights/[id]/route.ts` |
| F9–F9e | Restaurants POST/PATCH 500s | Fixed across routes — explicit payloads, NOT NULL defaults, check constraint |
| A1 | Flights gcal_include enabled on new records with no dates | Fixed in `FlightsClient.tsx` |
| B1 | Flights GET route missing new columns — refetch wiped new field values | Fixed: all new columns added to GET select in `app/api/trips/[id]/flights/route.ts` |
| B2 | Button.tsx `borderColor` conflict — 4 React warnings on rerender | Fixed: `borderColor` → four `borderXxxColor` longhands in Button.tsx + 7 section client files (Restaurants, Hotels, Packing, Transportation, KeyInfo, Checklist, Itinerary) |
